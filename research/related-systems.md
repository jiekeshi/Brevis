...
当前 smoke ablation 只能说明实现路径已打通，不能说明先验已学好：同模型校准的 PHOG 相对 uniform，BF16 归档缩小 0.011%，F32 和混合 I8 分别增大 0.00007% 和 0.00015%。因此现阶段应把 PHOG 表述为搜索顺序机制；压缩率收益必须等 held-out prior 实验后再下结论。相反，块并行解码已有清晰信号：三组模型相对 `--jobs 1` 分别加速 6.38×、6.15×、5.11×。原始结果固定在 `eval/results.json`。
